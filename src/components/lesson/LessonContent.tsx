                            ...
                            <div className='flex flex-col gap-3'>
                                {lecture.lessons.map((lesson) => (
                                    <LessonItem key={lesson._id}
                                        lesson={lesson ? JSON.parse(JSON.stringify(lesson)) : {}} url={!course ? "" : `/${course}/lesson?slug=${lesson.slug}`} isActive={!slug ? false : lesson.slug == slug} isChecked={histories.some(el => el.lesson.toString() == lesson._id.toString() )}></LessonItem>
                                ))}
                            </div>
                        </AccordionContent>
                    </AccordionItem>
